...
async function check() {
  ...
  const links1 = await tDb
    .select()
    .from(schema.purchaseDeliveryNoteLineBatches)
    .where(eq(schema.purchaseDeliveryNoteLineBatches.batchNum, batchNum1));
  console.log('Links for 213123123:', links1);
  const links2 = await tDb
    .select()
    .from(schema.purchaseDeliveryNoteLineBatches)
    .where(eq(schema.purchaseDeliveryNoteLineBatches.batchNum, batchNum2));
