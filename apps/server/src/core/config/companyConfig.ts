...
export async function setCompanyConfig(db: any, patch: Partial<CompanyConfig>): Promise<CompanyConfig> {
  ...
  for (const { key, value } of entries) {
    const existing = await db.select({ id: schema.systemConfigs.id })
