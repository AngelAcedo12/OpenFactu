...
import { FileDigit, Plus, Trash2, ArrowLeft, Save, ShoppingCart, FileText, AlertCircle, Copy, PlusSquare, Download } from 'lucide-react';
...
const POList: React.FC<{
 ...
 <div className="flex items-center justify-end gap-2">
   <Button
     variant="ghost"
     size="sm"
     onClick={(e) => { e.stopPropagation(); handleQuickPdf(item.id); }}
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2 font-medium ml-1">Gestión de aprovisionamiento y órdenes a proveedores.</p>
...
const POForm: React.FC<{
 ...
 <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Nuevo Pedido de Compra</h1>
 </div>
 <Button  onClick={onSubmit}  isLoading={state.isSubmitting}  disabled={!!state.seriesError || !state.canWrite}  className="px-8 flex items-center gap-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50" >
 ...
 <div className="space-y-2">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Almacén Destino *</label>
...
const PODetail: React.FC<{
 ...
 <div>
 <p className="font-bold text-slate-800 dark:text-slate-100">{masters.items.find((it: any) => it.id === l.itemId)?.name || 'Artículo'}</p>
 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-mono italic">{masters.items.find((it: any) => it.id === l.itemId)?.code}</p>
 </div>
 )},
 ...
 <span className={Number(l.receivedQty) > 0 ?"text-emerald-600 dark:text-emerald-300 font-bold":"text-slate-400 dark:text-slate-500"}>
 ...
 <PrintTemplateButton docType="PO"pdfUrl={`/api/purchases/orders/${order.id}/pdf`} label="PDF"/>
 {order.status !== 'C' && (
 ...
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest">Proveedor: <span className="text-slate-900 dark:text-slate-100 ml-2 font-bold">{masters.partners.find((p: any) => p.id === order.partnerId)?.name}</span></h4>
 </div>
 <div className="grid grid-cols-2 gap-6">
 <div className="space-y-1">
...
export const PurchaseOrders: React.FC = () => {
 ...
 return <POList doc={doc} data={orders} loading={loading} partners={doc.masters.partners} onCreate={() => setView('create')} canWrite={doc.state.canWrite} onDetail={async (p) => {  setLoading(true);
