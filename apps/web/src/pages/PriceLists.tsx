...
export const PriceLists: React.FC = () => {
 ...
 const filteredItems = items.filter(i =>  i.name.toLowerCase().includes(searchItem.toLowerCase()) ||  i.code.toLowerCase().includes(searchItem.toLowerCase())
 );
 ...
 <span className="text-[10px] font-black text-blue-600 dark:text-blue-300 uppercase tracking-[0.2em]">Comercial / Pricing</span>
 </div>
 <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Gestión de Tarifas</h1>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Controla tus márgenes y listas de precios de forma masiva.</p>
 </div>
 </header>
 ...
 <Card  noPadding  title="Catálogos Activos" headerAction={
 <Button  size="sm" onClick={() => setNewListRow({ name: '' })}  disabled={!!newListRow || !canWrite}
 ...
 <Input  placeholder="Ej: Mayoristas" value={newListRow.name}  onChange={e => setNewListRow({ name: e.target.value })}  className="h-9 text-sm" autoFocus
 />
 </td>
 ...
 {lists.map(l => (
 <tr  key={l.id}  onClick={() => { setSelectedList(l); fetchPrices(l.id); }}
 ...
 {editingListId === l.id ? (
 <Input  value={l.name}  onChange={e => setLists(lists.map(x => x.id === l.id ? { ...x, name: e.target.value } : x))}
 className="h-9 text-sm" autoFocus
 />
 ) : (
 <div className="flex items-center gap-3">
 <div className={`p-1.5 rounded-lg transition-colors ${selectedList?.id === l.id ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 group-hover:bg-blue-100 dark:bg-blue-500/20 group-hover:text-blue-600 dark:text-blue-300'}`}>
 <Tag size={14} />
 </div>
 <span className={`text-sm font-bold ${selectedList?.id === l.id ? 'text-blue-700 dark:text-blue-200' : 'text-slate-700 dark:text-slate-200'}`}>{l.name}</span>
 </div>
 )}
 </td>
 ...
 <button  onClick={(e) => { e.stopPropagation(); canWrite && setEditingListId(l.id); }}  disabled={!canWrite}
 ...
 <button  onClick={(e) => { e.stopPropagation(); canDelete && handleDeleteList(l.id); }}  disabled={!canDelete}
 ...
 <div>
 <h2 className="text-xl font-black text-slate-900 dark:text-slate-100 leading-tight">Precios: {selectedList.name}</h2>
 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-1">Asignación masiva de precios especiales</p>
 </div>
 <div className="relative">
 <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500"/>
 <input  placeholder="Buscador por código o nombre..." value={searchItem}
 ...
 return (
 <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 group transition-colors">
 <td className="px-6 py-3">
 <div className="flex flex-col">
 <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 tracking-tighter uppercase">{item.code}</span>
 <span className="text-sm font-bold text-slate-800 dark:text-slate-100 leading-tight">{item.name}</span>
 </div>
 </td>
 <td className="px-6 py-3 text-center">
 <span className="font-mono text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.basePrice}€</span>
 </td>
 <td className="px-6 py-3 text-center">
 {itemPrice ? (
 <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-black tracking-tighter border ${diff >= 0 ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-300 border-rose-100 dark:border-rose-500/20' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 border-emerald-100 dark:border-emerald-500/20'}`}>
 {diff >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
 {Math.abs(diff).toFixed(1)}% {diff >= 0 ? 'Recargo' : 'Dcto'}
 </div>
 ) : (
 <span className="text-slate-300 dark:text-slate-600 italic text-[10px]">Sin cambios</span>
 )}
 </td>
 <td className="px-6 py-3 text-right">
 <div className="flex items-center justify-end gap-2">
 <div className="relative w-32">
 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 dark:text-slate-600">€</span>
 <input  id={`price-input-${item.id}`}
 type="number" step="0.01" placeholder={item.basePrice}
 ...
 savingItems.includes(item.id)  ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'  : canWrite  ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-300 hover:bg-blue-600 hover:text-white shadow-sm'  : 'bg-slate-50 dark:bg-slate-800/50 text-slate-100 cursor-not-allowed grayscale'
 }`}
 >
 ...
 <tr>
 <td colSpan={4} className="p-20 text-center text-slate-300 dark:text-slate-600 italic">No hay artículos que coincidan con la búsqueda.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 <h3 className="text-lg font-black text-slate-700 dark:text-slate-200 tracking-tight">Potencia Comercial</h3>
 <p className="max-w-xs text-center font-medium mt-2 text-xs text-slate-400 dark:text-slate-500 leading-relaxed">Selecciona una lista de la izquierda para empezar a optimizar tus márgenes de beneficio de forma masiva.</p>
 </div>
 )}
 </div>
 </div>
