...
import { Truck, Plus, Trash2, ArrowLeft, Save, Copy, PlusSquare, Barcode, ShoppingCart, AlertCircle, Download } from 'lucide-react';
...
const SDNList: React.FC<{
 ...
 <span className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-mono mt-1">ID: {item.id.substring(0,8)}</span>
 </div>
 )},
 ...
 ) : <span className="text-[10px] text-slate-300 dark:text-slate-600 dark:text-slate-300 dark:text-slate-600 font-bold italic">Directo</span> },
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2 font-medium ml-1">Registro físico de salida de productos y control de expedición.</p>
...
const SDNForm: React.FC<{
 ...
 <div className="space-y-2">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Cliente *</label>
 <SearchableSelect
 ...
 <div className="space-y-2">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Almacén de Salida *</label>
 <SearchableSelect
 ...
 <div className="space-y-2">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Fecha Albarán *</label>
 ...
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 tracking-widest border-b pb-2">Logística y Series</h4>
...
const SDNDetail: React.FC<{
 ...
 <p className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-mono italic">{item?.code}</p>
 {hasTrace && (
 ...
 <div className="flex items-center gap-4">
 <button onClick={onBack} className="p-2 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-800 rounded-lg transition shadow-sm border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 dark:text-slate-500"><ArrowLeft size={20} /></button>
 <h1 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
 Albarán {sdn.seriesPrefix}-{sdn.periodCode}-{String(sdn.docNum).padStart(6, '0')}
 </h1>
 </div>
 <div className="flex gap-3">
 <PrintTemplateButton docType="SDN"pdfUrl={`/api/sales/delivery-notes/${sdn.id}/pdf`} label="PDF"/>
 {sdn.status === 'O' && (
 ...
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 tracking-widest">Cliente: <span className="text-slate-900 dark:text-slate-100 ml-2">{masters.partners.find((p: any) => p.id === sdn.partnerId)?.name}</span></h4>
 </div>
 {sdn.orderId && (
 ...
 <p className="text-xs font-bold text-blue-700 dark:text-blue-200 italic">Viene del pedido: {sdn.orderPrefix}-{sdn.periodCode}-{String(sdn.orderDocNum).padStart(6, '0')}</p>
 </div>
 )}
 </Card>

 <Card className="p-6 space-y-4 border-slate-100 dark:border-slate-800">
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 tracking-widest border-b pb-2">CRONOGRAMA</h4>
 <div className="space-y-2">
 <div className="flex justify-between text-sm"><span className="text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 italic">Fecha:</span> <span className="font-bold">{new Date(sdn.date).toLocaleDateString()}</span></div>
 <div className="flex justify-between text-sm pt-2 border-t"><span className="text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 italic">Estado:</span> {sdn.status === 'O' ? <Badge variant="warning">Abierto</Badge> : <Badge variant="success">Facturado</Badge>}</div>
 </div>
 </Card>
 </div>
...
export const SalesDeliveryNotes: React.FC = () => {
 ...
 const doc = useDocument({  token: token || '',  tenantId: user?.tenantId || '',  docType: 'SDN',  apiEndpoint: '/api/sales/delivery-notes',
 permissions: user?.permissions?.[location.pathname]
 });
 ...
 return <SDNList doc={doc} data={deliveries} loading={loading} partners={doc.masters.partners} onCreate={() => setView('create')} onDetail={async (p) => {  setLoading(true);
