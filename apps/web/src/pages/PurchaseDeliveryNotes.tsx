...
import { Truck, Plus, Trash2, ArrowLeft, Save, ShoppingCart, Copy, ClipboardList, FileText, AlertCircle, PlusSquare, FileDigit, Barcode } from 'lucide-react';
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium ml-1">Registro físico de entrada de productos y trazabilidad.</p>
        </div>
  ...
  const columns = [
    ...
    { header: '', align: 'center' as const, cell: (_: any, idx: number) => (
      <div className="flex items-center gap-1">
        ...
        }} className="text-slate-300 hover:text-blue-500" title="Duplicar">
          <Copy size={14} />
        </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 md:col-span-2 space-y-6 shadow-xl border-t-4 border-t-emerald-500">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Proveedor *</label>
                <SearchableSelect
              ...
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Almacén de Entrada *</label>
                <SearchableSelect
        ...
        <div className="p-6 bg-slate-50/50 flex flex-col md:flex-row justify-between items-start md:items-center border-t border-slate-200 gap-6">
           <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
                 <button onClick={() => Array(1).fill(0).forEach(actions.addLine)} className="h-8 min-w-[36px] px-2 rounded-lg text-[10px] font-black bg-white hover:bg-slate-50 text-slate-600 border border-transparent hover:border-slate-200 transition-all">+1</button>
                 <button onClick={() => Array(5).fill(0).forEach(actions.addLine)} className="h-8 min-w-[36px] px-2 rounded-lg text-[10px] font-black bg-white hover:bg-slate-50 text-slate-600 border border-transparent hover:border-slate-200 transition-all">+5</button>
                 <button onClick={() => Array(10).fill(0).forEach(actions.addLine)} className="h-8 min-w-[36px] px-2 rounded-lg text-[10px] font-black bg-white hover:bg-slate-50 text-slate-600 border border-transparent hover:border-slate-200 transition-all">+10</button>
              </div>
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-in slide-in-from-right duration-300">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          {pdn.status === 'O' && (
            <>
              <Button variant="secondary" onClick={() => onCancel(pdn.id)} className="text-rose-600 border-rose-100 hover:bg-rose-50">Cancelar</Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 md:col-span-2 space-y-6 shadow-xl border-slate-100">
          <div className="flex justify-between items-start border-b pb-3">
             <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Proveedor: <span className="text-slate-900 ml-2">{masters.partners.find((p: any) => p.id === pdn.partnerId)?.name}</span></h4>
             <Badge variant="neutral" className="text-[10px]">ID: {pdn.partnerId.substring(0,8)}</Badge>
          </div>
          {pdn.orderId && (
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-center gap-3">
              <ShoppingCart size={18} className="text-blue-500" />
              <p className="text-xs font-bold text-blue-700 italic">Viene del pedido: {pdn.orderPrefix}-{pdn.periodCode}-{String(pdn.orderDocNum).padStart(6, '0')}</p>
            </div>
          )}
          <p className="text-sm text-slate-500 italic">Este albarán ha generado movimientos de stock en el almacén destino.</p>
        </Card>
        ...
        <Card className="p-6 space-y-4 shadow-xl border-slate-100">
           ...
           <div className="space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-400 italic">Fecha:</span> <span className="font-bold">{new Date(pdn.date).toLocaleDateString()}</span></div>
              <div className="flex justify-between text-sm pt-2 border-t"><span className="text-slate-400 italic">Estado:</span> {pdn.status === 'O' ? <Badge variant="warning">Abierto</Badge> : <Badge variant="success">Facturado</Badge>}</div>
           </div>
        </Card>
      </div>
...
export const PurchaseDeliveryNotes: React.FC = () => {
  ...
  const handleCancel = async (id: string) => {
    if (!confirm('¿Seguro que deseas cancelar este albarán?')) return;
    try {
  ...
  return <PDNList data={deliveries} loading={loading} partners={doc.masters.partners} onCreate={() => setView('create')} onDetail={async (p) => { 
    setLoading(true);
