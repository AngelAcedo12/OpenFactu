...
export const DocumentSeries: React.FC = () => {
 ...
 body: JSON.stringify({  name,  description: `Serie ${name}`,
 periodId,
 docType,
 ...
 {  header: 'Rango Visual',  cell: (c: any) => (
 <span className="font-mono text-xs">
 {c.prefix ? `${c.prefix}-` : ''}{c.firstNumber} ... {c.prefix ? `${c.prefix}-` : ''}{c.lastNumber}
 </span>
 )
 },
 {  header: 'Siguiente Num',  cell: (c: any) => <Badge variant="neutral"className="font-mono">{c.nextNumber}</Badge>
 },
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1 font-medium">Configura los rangos de numeración para cada tipo de documento del sistema.</p>
 </div>
 ...
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Nombre Serie</label>
 <Input  placeholder="Ej: Principal PO" value={name}
 onChange={(e) => setName(e.target.value)}
 required
 />
 </div>
 <div className="md:col-span-1">
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Tipo Documento</label>
 ...
 <div className="md:col-span-1">
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Aplica al Periodo</label>
 ...
 <div className="md:col-span-1">
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Prefijo Visual</label>
 <Input  placeholder="Ej: F24" value={prefix}
 onChange={(e) => setPrefix(e.target.value)}
 />
 </div>
 <div className="md:col-span-1">
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Inicio de Rango</label>
 ...
 <div className="md:col-span-1">
 <label className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Límite Final</label>
 <Input  type="number" value={lastNumber}
 onChange={(e) => setLastNumber(Number(e.target.value))}
 required
 />
 </div>
 <div className="md:col-span-2 flex justify-end">
 <Button  type="submit" disabled={isSubmitting || periods.length === 0 || !canWrite}  className="flex items-center gap-2 px-8 disabled:opacity-50 disabled:grayscale transition-all" >
