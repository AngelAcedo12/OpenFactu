...
import { Table, Card, Button, Input, useToast, Badge, FilterBar, SearchableSelect, cn } from '@openfactu/ui';
import { useAuth } from '../context/AuthContext';
import { FileStack, Plus, Trash2, ArrowLeft, Save, FileText, AlertCircle, Copy, PlusSquare, Barcode, Download } from 'lucide-react';
...
const InvoiceList: React.FC<{  data: any[],  loading: boolean,  partners: any[],  onCreate: () => void,
 ...
 { header: 'Proveedor', accessor: (item: any) => (
 <div>
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2 font-medium ml-1">Registro para el libro de IVA y pagos a proveedores.</p>
 {mastersError && (
...
const InvoiceForm: React.FC<{
 ...
 <div className="space-y-2">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Proveedor / Acreedor *</label>
 <SearchableSelect
...
const InvoiceDetail: React.FC<{
 ...
 setViewingBatch: (l: any) => void;
 canDelete?: boolean;
}> = ({ invoice, onBack, onCancel, masters, setViewingBatch, canDelete }) => {
 const columns = [
 ...
 <header className="flex items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
 <div className="flex items-center gap-4 min-w-0">
 <button
 onClick={onBack}
 className="p-2.5 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm transition-all text-slate-600 dark:text-slate-300 shrink-0" aria-label="Volver" >
 <ArrowLeft size={18} />
 </button>
 <div className="min-w-0">
 <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-0.5 flex items-center gap-1.5">
 <FileStack size={11} /> Compras · Factura
 </p>
 <div className="flex items-center gap-3 flex-wrap">
 <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight font-display truncate">
 {invoice.seriesPrefix}-{invoice.periodCode}-{String(invoice.docNum).padStart(6, '0')}
 </h1>
 {invoice.status === 'O'
 ? <Badge variant="success"className="uppercase tracking-wide">Asentado</Badge>
 : <Badge variant="error"className="uppercase tracking-wide">Cancelado</Badge>}
 </div>
 </div>
 </div>
 <div className="flex items-center gap-2 shrink-0">
 <PrintTemplateButton docType="PINV"pdfUrl={`/api/purchases/invoices/${invoice.id}/pdf`} />
 {invoice.status === 'O' && (
 <Button
 variant="secondary" onClick={() => onCancel(invoice.id)}
 disabled={!canDelete}
 className="flex items-center gap-2 border border-rose-100 dark:border-rose-500/30 text-rose-600 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-500/10 disabled:opacity-30" >
 ...
 <div>
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-1">PROVEEDOR / ACREEDOR</h4>
 ...
 <Badge variant="neutral"className="font-mono text-[10px]">ID: {invoice.partnerId.substring(0,8)}</Badge>
 </div>
 ...
 <p className="text-xs text-amber-800 dark:text-amber-200 font-medium">Este documento es una factura contable firme y genera obligaciones de pago.</p>
 </div>
 </Card>
 ...
 <span className="text-slate-400 dark:text-slate-500 italic">Fecha:</span>
 <span className="font-bold text-slate-800 dark:text-slate-100">{new Date(invoice.date).toLocaleDateString()}</span>
 </div>
 <div className="flex justify-between text-sm pt-2 border-t">
 <span className="text-slate-400 dark:text-slate-500 italic">Serie:</span>
 <span className="font-bold text-slate-700 dark:text-slate-200">
 {masters.series.find((s: any) => s.id === invoice.seriesId)?.name || invoice.seriesPrefix || '--'}
 </span>
 </div>
 </div>
 </Card>
 ...
 {invoice.lines?.[0]?.baseId && (
 <p className="text-[10px] font-bold text-blue-600 dark:text-blue-300 italic mt-3 flex items-center gap-1"><Copy size={12} /> Proviene de Albaranes</p>
 )}
 </Card>
 </div>
 </div>
 ...
 <span>Base Imponible</span>
 <span>{Number(invoice.subtotal || invoice.total).toFixed(2)} €</span>
 </div>
 ...
 <span className="text-slate-400 dark:text-slate-500 font-black uppercase text-[10px]">Total</span>
...
export const PurchaseInvoices: React.FC = () => {
 ...
 // Mapeamos los datos del albarán a la factura
 doc.setState.setPartnerId(pdn.partnerId);
 doc.setState.setLines(pdn.lines.map((l: any) => ({
 itemId: l.itemId,
 quantity: l.quantity,
 price: l.price,
 ...
 zoneId: l.zoneId,
 batchDetails: l.batchDetails, // <- REINSTAURADO TRASPASO DE LOTES
 baseType: 'PDN',
 baseId: pdn.id,
 baseLine: l.lineNum || l.id
 })));
 setView('create');
 // Limpiamos la URL para evitar recargas infinitas
 ...
 if (flags.confirmBeforeCancel && !confirm('¿Estás seguro de cancelar esta factura? Los albaranes se reabrirán.')) return;
 try {
 ...
 <>
 <InvoiceDetail invoice={selectedInvoice} onBack={() => setView('list')} onCancel={handleCancel} masters={doc.masters} setViewingBatch={setViewingBatch} canDelete={doc.state.canDelete} />
 {viewingBatch && (
 <BatchSelectionModal  isOpen={true} onClose={() => setViewingBatch(null)}
 targetQuantity={viewingBatch.quantity}
 itemName={doc.masters.items.find((i: any) => i.id === viewingBatch.itemId)?.name || ''}
 ...
 return <InvoiceList doc={doc} data={invoices} loading={loading} partners={doc.masters.partners} mastersError={doc.state.mastersError} onCreate={() => setView('create')} onDetail={async (inv) => {  setLoading(true);
 try {
 const res = await fetch(`/api/purchases/invoices/${inv.id}`, {
 headers: { 'Authorization': `Bearer ${token}`, 'x-tenant-id': user?.tenantId || '' }
 });
 const data = await res.json();
