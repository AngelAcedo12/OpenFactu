...
import { FileStack, Plus, Trash2, ArrowLeft, Save, ShoppingCart, FileText, AlertCircle, Copy, PlusSquare, FileDigit, Barcode } from 'lucide-react';
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium ml-1">Registro para el libro de IVA y pagos a proveedores.</p>
        </div>
...
const InvoiceDetail: React.FC<{
  ...
  masters: any;
  setViewingBatch: (l: any) => void;
}> = ({ invoice, onBack, onCancel, masters, setViewingBatch }) => {
  const columns = [
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-in slide-in-from-right duration-300">
      <div className="flex items-center justify-between">
        ...
        <div>
          {invoice.status === 'O' && (
            <Button variant="secondary" onClick={() => onCancel(invoice.id)} className="flex items-center gap-2 border-rose-100 text-rose-600 hover:bg-rose-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 md:col-span-2 space-y-6 shadow-xl border-slate-100">
           ...
           <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-xl">
               ...
               <p className="text-xs text-amber-800 font-medium">Este documento es una factura contable firme y genera obligaciones de pago.</p>
           </div>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 bg-slate-50 border border-slate-200">
             ...
             {invoice.lines?.[0]?.baseId && (
                <p className="text-[10px] font-bold text-blue-600 italic mt-3 flex items-center gap-1"><Copy size={12} /> Proviene de Albaranes</p>
             )}
          </Card>
        </div>
      </div>
...
export const PurchaseInvoices: React.FC = () => {
  ...
  const doc = useDocument({ 
    token: token || '', tenantId: user?.tenantId || '', docType: 'PINV', apiEndpoint: '/api/purchases/invoices' 
  });
  ...
  const handleCancel = async (id: string) => {
    if (!confirm('¿Estás seguro de cancelar esta factura? Los albaranes se reabrirán.')) return;
    try {
  ...
  if (view === 'detail' && selectedInvoice) return (
    <>
      <InvoiceDetail invoice={selectedInvoice} onBack={() => setView('list')} onCancel={handleCancel} masters={doc.masters} setViewingBatch={setViewingBatch} />
      {viewingBatch && (
        <BatchSelectionModal 
           isOpen={true} onClose={() => setViewingBatch(null)}
           targetQuantity={viewingBatch.quantity}
