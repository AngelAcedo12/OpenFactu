...
export const Warehouses: React.FC = () => {
 ...
 {  header: 'Centro Logístico',  accessor: (w: any) => (
 <div className="flex flex-col">
 <span className="font-bold text-slate-800 dark:text-slate-100">{w.name}</span>
 <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase">{w.location || 'Sin Ubicación'}</span>
 </div>
 )
 },
 {  header: 'Predet.',  accessor: (w: any) => w.isDefault && <div className="w-2 h-2 rounded-full bg-emerald-500"/>
 },
 {  header: 'Acciones',  align: 'right' as const,
 accessor: (w: any) => (
 <Button  variant={selectedWarehouse?.id === w.id ?"primary":"secondary"}  size="sm" onClick={() => setSelectedWarehouse(w)}
 >
 <ArrowRight size={14} />
 </Button>
 )
 ...
 const binColumns = [
 {  header: 'Ubicación / Bin',  accessor: (b: any) => (
 ...
 <span className="text-[11px] text-slate-400 dark:text-slate-500 font-medium italic">{b.description}</span>
 </div>
 )
 },
 {  header: 'Acciones',  align: 'right' as const,
 accessor: (b: any) => (
 <button  onClick={() => canDelete && deleteBin(b.id)}  disabled={!canDelete}
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1 font-medium text-sm">Gestión industrial de almacenes y mallas de ubicación.</p>
 </div>
 </header>
 ...
 <div>
 <Card  title="Centros Activos" noPadding
 headerAction={
 <Button  size="sm" onClick={() => setShowWarehouseModal(true)}  disabled={!canWrite}
 ...
 <p className="text-[10px] uppercase font-black text-slate-400 dark:text-slate-500 tracking-widest">{selectedWarehouse.location || 'BODEGA ESTÁNDAR'}</p>
 </div>
 </div>
 <div className="flex gap-2">
 <Button  onClick={() => setShowGenerator(true)}  disabled={!canWrite}
 ...
 <Card  noPadding  title="Malla de Ubicaciones" subtitle={`Total: ${bins.length} ubicaciones asignadas a este centro.`}
 ...
 <BinGeneratorModal  warehouseId={selectedWarehouse.id}  onClose={() => setShowGenerator(false)}  onSuccess={() => {
 ...
 title="Nuevo Almacén" subtitle="Define un nuevo centro de distribución." maxWidth="md" >
 ...
 <Input label="Ubicación Geográfica"value={whLocation} onChange={(e) => setWhLocation(e.target.value)} placeholder="Ej: Planta 2, Sector Sur"/>
