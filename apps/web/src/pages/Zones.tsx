...
export const Zones: React.FC = () => {
 ...
 <ArrowLeft size={20} className="text-slate-600 dark:text-slate-300 dark:text-slate-600"/>
 </button>
 ...
 <span className="text-[10px] font-black text-blue-600 dark:text-blue-300 uppercase tracking-[0.2em]">Logística / Almacenes</span>
 </div>
 <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight text-display">Zonas Logísticas</h1>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Agrupaciones territoriales para la gestión de stock y pasillos.</p>
 </div>
 </div>
 <Button  onClick={() => setNewRow({ name: '', description: '' })}  disabled={!!newRow || !canWrite}  className="flex items-center gap-2 disabled:opacity-50 disabled:grayscale transition-all" >
 ...
 {zones.map(z => (
 <tr key={z.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
 ...
 <p className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight">{z.name}</p>
 </div>
 )}
 </td>
 <td className="px-6 py-3">
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-xs font-medium">{z.description || <span className="text-slate-300 dark:text-slate-600 italic">Sin descripción</span>}</p>
 )}
 </td>
 ...
 <>
 <button  onClick={() => canWrite && setEditingId(z.id)}  disabled={!canWrite}
 ...
 <button  onClick={() => canDelete && handleDelete(z.id)}  disabled={!canDelete}
