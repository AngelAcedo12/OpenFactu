...
export const Taxes: React.FC = () => {
 ...
 <span className="text-[10px] font-black text-amber-600 dark:text-amber-300 uppercase tracking-[0.2em]">Finanzas / Configuración</span>
 </div>
 <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Gestión de Impuestos</h1>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Configura los tipos de IVA y retenciones aplicables a tus documentos.</p>
 </div>
 <Button  onClick={() => setNewRow({ code: '', rate: '' })}  disabled={!!newRow || !canWrite}  className="flex items-center gap-2 disabled:opacity-50 disabled:grayscale transition-all" >
 ...
 <td className="p-4">
 <Input
 placeholder="Ej: IVA_21" value={newRow.code}
 onChange={e => setNewRow({ ...newRow, code: e.target.value })}
 className="h-10" />
 </td>
 <td className="p-4">
 <Input
 type="number" placeholder="21" value={newRow.rate}
 ...
 <Button size="sm"variant="secondary"onClick={() => setNewRow(null)} className="h-10"><X size={14}/></Button>
 </td>
 </tr>
 )}
 ...
 <p className="text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold uppercase">Identificador Maestro</p>
 </div>
 </div>
 )}
 </td>
 ...
 <>
 <Button  size="sm" variant="secondary" onClick={() => canWrite && setEditingId(t.id)}  disabled={!canWrite}
 ...
 <Button  size="sm" variant="secondary" onClick={() => canDelete && handleDelete(t.id)}  disabled={!canDelete}
 ...
 <Loader size="lg"/>
 <p className="text-slate-400 dark:text-slate-500 mt-4 font-medium italic">Sincronizando con el servidor...</p>
 </td>
 </tr>
 )}
 </tbody>
