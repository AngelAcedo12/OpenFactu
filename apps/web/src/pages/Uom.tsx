...
export const Uom: React.FC = () => {
 ...
 <span className="text-[10px] font-black text-blue-600 dark:text-blue-300 uppercase tracking-[0.2em]">Logística / Maestro</span>
 </div>
 <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight text-display">Unidades de Medida</h1>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Define las dimensiones y conversiones globales para tus artículos.</p>
 </div>
 <Button  onClick={() => setNewRow({ name: '', symbol: '', baseValue: '1.0000', baseUomId: null })}  disabled={!!newRow || !canWrite}  className="flex items-center gap-2 disabled:opacity-50 disabled:grayscale" >
 ...
 <select  value={newRow.baseUomId || ''}  onChange={e => setNewRow({ ...newRow, baseUomId: e.target.value || null })}
 ...
 <div>
 <p className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight">{u.name}</p>
 <Badge variant="neutral"className="mt-1 font-mono uppercase tracking-widest text-[9px] bg-slate-100 dark:bg-slate-800">{u.code || '---'}</Badge>
 </div>
 </div>
 )}
 </td>
 ...
 <select  value={u.baseUomId || ''}  onChange={e => setUoms(uoms.map(x => x.id === u.id ? { ...x, baseUomId: e.target.value || null } : x))}
 className="h-9 bg-white dark:bg-slate-900 border rounded-lg px-2 text-xs" >
 <option value="">(Unidad Primaria)</option>
 ...
 <ArrowRightLeft size={10} className="text-slate-300 dark:text-slate-600"/>
 ...
 <>
 <button  onClick={() => canWrite && setEditingId(u.id)}  disabled={!canWrite}
