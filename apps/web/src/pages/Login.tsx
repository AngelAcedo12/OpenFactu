...
export const Login: React.FC = () => {
 ...
 if (!emailOrUsername.trim()) { setTenants([]); setSelectedTenant(''); return; }
 try {
 const res = await fetch(`/api/memberships/tenants-for-user?email=${encodeURIComponent(emailOrUsername)}`);
 ...
 <img  src="/assets/login_bg.png" alt="Business Background" className="w-full h-full object-cover opacity-60 scale-105" />
 ...
 <p className="text-lg text-slate-300 dark:text-slate-600 max-w-sm leading-relaxed font-medium">
 ...
 <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Lógica compartida y reactividad instantánea en todas tus operaciones.</p>
 </div>
 </div>
 <div className="flex items-start gap-4 group">
 ...
 <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Aislamiento total de datos en esquemas dedicados de base de datos.</p>
 </div>
 </div>
 </div>
 </div>

 <div className="flex items-center justify-between text-slate-500 dark:text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest pt-12 border-t border-white/10">
 ...
 <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">OpenFactu</h1>
 </div>

 <div className="space-y-2">
 <h3 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Iniciar Sesión</h3>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Introduce tus credenciales para acceder al ERP.</p>
 </div>
 ...
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Usuario / Email</label>
 ...
 placeholder="admin o usuario@empresa.com" className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-slate-900 dark:text-slate-100 text-sm placeholder:text-slate-300 dark:text-slate-600 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-semibold shadow-sm" />
 </div>
 </div>

 {/* Selector de Empresa — aparece después de introducir el usuario */}
 {tenants.length > 1 && (
 <div className="space-y-1.5 focus-within:translate-y-[-2px] transition-transform animate-in fade-in slide-in-from-top-2 duration-300">
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Empresa</label>
 <div className="relative group">
 <div className="absolute inset-y-0 left-4 flex items-center text-slate-400 dark:text-slate-500 group-focus-within:text-blue-600 dark:text-blue-300 transition-colors pointer-events-none">
 ...
 className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 pl-12 pr-10 text-slate-900 dark:text-slate-100 text-sm appearance-none focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all cursor-pointer font-semibold shadow-sm" required
 >
 ...
 <div className="absolute inset-y-0 right-4 flex items-center text-slate-400 dark:text-slate-500 pointer-events-none">
 <ChevronDown size={18} />
 </div>
 </div>
 </div>
 ...
 <span className="text-sm font-bold text-blue-700 dark:text-blue-200">{tenants[0].name}</span>
 </div>
 )}
 ...
 <label className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Contraseña</label>
 <button type="button"className="text-[11px] font-bold text-blue-600 dark:text-blue-300 hover:text-blue-700 dark:text-blue-200 hover:underline transition">Olvido su contraseña?</button>
 </div>
 <div className="relative group">
 <div className="absolute inset-y-0 left-4 flex items-center text-slate-400 dark:text-slate-500 group-focus-within:text-blue-600 dark:text-blue-300 transition-colors pointer-events-none">
 <Lock size={18} />
 ...
 className="w-full bg-slate-950 hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed text-white font-black py-4.5 rounded-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 group tracking-tight" >
 {isSubmitting ? (
 ...
 Dudas con tu acceso? <button type="button"className="text-blue-600 dark:text-blue-300 hover:underline">Contactar soporte</button>
 </p>
 </div>
 </form>
 ...
 <p className="absolute bottom-8 text-slate-300 dark:text-slate-600 text-[10px] font-black uppercase tracking-[0.2em] pointer-events-none">
