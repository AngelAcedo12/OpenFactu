...
export const StyleGuide: React.FC = () => {
 ...
 <Badge variant={item.status === 'Pagado' ? 'success' : item.status === 'Pendiente' ? 'warning' : 'error'}>
 {item.status}
 </Badge>
 )
 },
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2">Librería de componentes genéricos para OpenFactu.</p>
 </header>
 ...
 <Input label="Campo con Error"error="Este campo es obligatorio"defaultValue="Contenido no válido"/>
 </div>
 </Card>
 ...
 <div className="flex flex-wrap gap-4">
 <Button onClick={() => toast.success('Operación completada con éxito')} variant="secondary"className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-200 border-emerald-200 hover:bg-emerald-100">
 Lanzar Éxito
 </Button>
 <Button onClick={() => toast.error('Ha ocurrido un error inesperado')} variant="secondary"className="bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-200 border-rose-200 hover:bg-rose-100">
 Lanzar Error
 </Button>
 <Button onClick={() => toast.warning('Tu suscripción está por expirar')} variant="secondary"className="bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-200 border-amber-200 hover:bg-amber-100">
 Lanzar Aviso
 </Button>
 <Button onClick={() => toast.info('Nueva actualización disponible')} variant="secondary"className="bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-200 border-blue-200 hover:bg-blue-100 dark:bg-blue-500/20">
 Lanzar Info
 </Button>
 </div>
 </Card>
