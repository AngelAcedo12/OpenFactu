...
export const PartnerGroups: React.FC = () => {
 ...
 <span className="text-[10px] font-black text-blue-600 dark:text-blue-300 uppercase tracking-[0.2em]">CRM / Estructura</span>
 </div>
 <h1 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight text-display">Grupos de Socios</h1>
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">Clasifica tus interlocutores comerciales para segmentación y tarifas.</p>
 </div>
 <Button  onClick={() => setNewRow({ code: '', name: '', codePrefix: '', isCustomer: false, isVendor: false })}  disabled={!!newRow || !canWrite}  className="flex items-center gap-2 disabled:opacity-50 disabled:grayscale transition-all" >
 ...
 <input type="checkbox"checked={newRow.isCustomer} onChange={e => setNewRow({ ...newRow, isCustomer: e.target.checked })} className="rounded text-blue-600 dark:text-blue-300"/>
 <span>Cliente</span>
 </label>
 <label className="flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 dark:text-slate-600 cursor-pointer">
 <input type="checkbox"checked={newRow.isVendor} onChange={e => setNewRow({ ...newRow, isVendor: e.target.checked })} className="rounded text-blue-600 dark:text-blue-300"/>
 <span>Proveedor</span>
 </label>
 </div>
 </td>
 ...
 {groups.map(g => (
 <tr key={g.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
 ...
 <div>
 <p className="font-bold text-slate-800 dark:text-slate-100 text-sm leading-tight">{g.name}</p>
 <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">{g.code}</p>
 </div>
 </div>
 )}
 </td>
 ...
 g.codePrefix ? (
 <Badge variant="neutral"className="font-mono tracking-widest text-[10px] bg-slate-100 dark:bg-slate-800">{g.codePrefix}-XXX</Badge>
 ) : (
 ...
 <label className="flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 dark:text-slate-600 cursor-pointer">
 <input type="checkbox"checked={g.isCustomer} onChange={e => setGroups(groups.map(x => x.id === g.id ? { ...x, isCustomer: e.target.checked } : x))} className="rounded text-blue-600 dark:text-blue-300"/>
 <span>Cliente</span>
 </label>
 <label className="flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 dark:text-slate-600 cursor-pointer">
 <input type="checkbox"checked={g.isVendor} onChange={e => setGroups(groups.map(x => x.id === g.id ? { ...x, isVendor: e.target.checked } : x))} className="rounded text-blue-600 dark:text-blue-300"/>
 <span>Proveedor</span>
 </label>
 </div>
 ) : (
 ...
 <>
 <button  onClick={() => canWrite && setEditingId(g.id)}  disabled={!canWrite}
 ...
 <button  onClick={() => canDelete && handleDelete(g.id)}  disabled={!canDelete}
