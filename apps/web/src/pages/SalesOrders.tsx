...
import { FileDigit, Plus, Trash2, ArrowLeft, Save, Copy, PlusSquare, Barcode } from 'lucide-react';
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium ml-1">Gestión de preventas y órdenes de clientes.</p>
        </div>
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Nuevo Pedido de Venta</h1>
        </div>
...
const SODetail: React.FC<{
  ...
  const columns = [
    ...
    { header: 'Entregado', align: 'center' as const, cell: (l: any) => (
      <span className={Number(l.deliveredQty) > 0 ? "text-emerald-600 font-bold" : "text-slate-400"}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 md:col-span-2 space-y-6 shadow-xl border-slate-100">
          <div className="flex justify-between items-start border-b pb-3">
             <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Cliente: <span className="text-slate-900 ml-2 font-bold">{masters.partners.find((p: any) => p.id === order.partnerId)?.name}</span></h4>
          </div>
          <div className="grid grid-cols-2 gap-6">
             <div className="space-y-1">
...
export const SalesOrders: React.FC = () => {
  ...
  if (view === 'detail' && selectedOrder) return (
    <>
      <SODetail order={selectedOrder} onBack={() => setView('list')} onCopyToDelivery={() => { localStorage.setItem('copy_order_source', JSON.stringify(selectedOrder)); window.location.href = `/sales/delivery-notes?copyFrom=${selectedOrder.id}`; }} masters={doc.masters} setViewingBatch={setViewingBatch} />
      {viewingBatch && (
        <BatchSelectionModal 
           isOpen={true} onClose={() => setViewingBatch(null)}
  ...
  return <SOList data={orders} loading={loading} partners={doc.masters.partners} onCreate={() => setView('create')} onDetail={async (p) => { 
    setLoading(true);
