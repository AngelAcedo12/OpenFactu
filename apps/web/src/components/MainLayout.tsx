...
import {  BarChart3,  Package,  Settings,  MapPin,  Users,  LogOut,  Layers,  ChevronDown,  ChevronRight,
 Database,
 Grid,
 Hash,
 Boxes,
...
const NavItem: React.FC<NavItemProps> = ({ icon: Icon, label, path, isActive, onClick, className, isCustomIcon }) => (
 <button
 onClick={onClick}
 className={cn(
"w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all group relative overflow-hidden",
 isActive
 ?"bg-primary/10 text-primary shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]" :"text-slate-400 dark:text-slate-500 hover:bg-slate-800/50 hover:text-slate-200",
 className
 )}
 >
 {isActive && (
 ...
 {isCustomIcon ? (
 <PluginIcon iconName={Icon} size={18} className={cn("transition-transform group-hover:scale-110", isActive ?"text-primary":"opacity-70")} />
 ) : (
 <Icon size={18} className={cn("transition-transform group-hover:scale-110", isActive ?"text-primary":"opacity-70")} />
 )}
 <span className="truncate">{label}</span>
 {!isActive && (
 <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
 <ChevronRight size={12} className="text-slate-600 dark:text-slate-300 dark:text-slate-600"/>
 </div>
 )}
 </button>
);
...
export const MainLayout: React.FC = () => {
 ...
 const pluginMenuItems = manifests.flatMap(m =>  (m.ui?.menuItems || []).map(item => ({
 group: 'extensiones',
 icon: item.icon,
 label: item.label,
 path: item.path,
 ...
 <div>
 <h1 className="text-lg font-black text-white leading-none tracking-tighter font-display">{branding.appName}</h1>
 <p className="text-[10px] text-primary font-black uppercase tracking-widest mt-0.5">Enterprise</p>
 </div>
 </div>
 </div>
 ...
 <svg className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400 dark:text-slate-500 group-focus-within/search:text-blue-500 transition-colors"fill="none"viewBox="0 0 24 24"stroke="currentColor">
 <path strokeLinecap="round"strokeLinejoin="round"strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
 </svg>
 </div>
 <input  type="text" value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 placeholder="Buscar en menú..." className="block w-full pl-9 pr-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-xl text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all font-medium" />
 </div>
 </div>
 ...
 id={group.id}  label={group.label}  icon={group.icon}  isOpen={isGroupOpen}  onToggle={toggleGroup}
 >
 {itemsInGroup.map(item => (
 <NavItem  key={item.path}
 icon={item.icon}  label={item.label}  path={item.path}  isActive={isActive(item.path)}  onClick={() => navigate(item.path)}  isCustomIcon={(item as any).isCustom}
 />
 ))}
 ...
 <p className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold uppercase truncate tracking-widest">{user?.role || 'USER'}</p>
 </div>
 </div>
 <button
 onClick={logout}
 ...
 <div className="relative">
 <button onClick={() => navigate('/purchase-orders')} className="flex items-center gap-2 bg-slate-900 dark:bg-primary text-white dark:text-primary-fg px-4 py-2 rounded-lg text-sm font-bold shadow-md hover:bg-slate-800 dark:hover:bg-primary-hover hover:shadow-lg transition-all active:scale-95">
 <FileDigit size={16} />
...
"px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-[0.15em]",
 color === 'emerald' ?"bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 border border-emerald-100 dark:border-emerald-500/20":"bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500" )}>
 {label}
 </div>
);
