...
export const BatchSelectionModal: React.FC<Props> = ({  isOpen,  onClose,  onConfirm,  targetQuantity,  itemName,
 initialDetails = [],
 manageBy,
 readOnly = false,
 itemId,
 ...
 setLoading(true);
 console.log(`[FRONTEND-DIAG] Buscando lotes para Item: ${itemId} | Tenant: ${user?.tenantId}`);
 ...
 <div>
 <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Cantidad Requerida</p>
 <p className="text-2xl font-black text-slate-800 dark:text-slate-100">{targetQuantity}</p>
 </div>
 <div className="text-right">
 <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Total Asignado</p>
 ...
 {isSale && !readOnly ? (
 <select  value={d.batchNum}  onChange={e => updateLine(idx, 'batchNum', e.target.value)}
 ...
 <option value="">Seleccionar {manageBy === 'B' ? 'lote' : 'serie'}...</option>
 {availableBatches.map(ab => {
 const location = ab.warehouseName  ? ` [${ab.warehouseName}${ab.zoneName ? ` - ${ab.zoneName}` : ''}]`
 ...
 <Input
 placeholder={manageBy === 'B' ?"Lote...":"Serie..."}  value={d.batchNum}  onChange={e => updateLine(idx, 'batchNum', e.target.value)}
 disabled={readOnly}
 className={cn("h-9 border-slate-200 dark:border-slate-700", readOnly ?"bg-white dark:bg-slate-900 border-transparent font-bold text-slate-700 dark:text-slate-200":"focus:border-blue-500")}
 />
 )}
 </td>
 <td className="py-2 px-2">
 <Input  type="number" value={d.quantity}  onChange={e => updateLine(idx, 'quantity', Number(e.target.value))}
 disabled={manageBy === 'S' || readOnly}
 className={cn("h-9 text-center border-slate-200 dark:border-slate-700", readOnly &&"bg-white dark:bg-slate-900 border-transparent font-bold text-slate-700 dark:text-slate-200")}
 />
 </td>
 {manageBy === 'B' && (
 <td className="py-2 px-2">
 <Input  type="date" value={d.expiryDate || ''}  onChange={e => updateLine(idx, 'expiryDate', e.target.value)}
 disabled={readOnly}
 className={cn("h-9 border-slate-200 dark:border-slate-700", readOnly &&"bg-white dark:bg-slate-900 border-transparent font-bold text-slate-700 dark:text-slate-200")}
 />
 </td>
 )}
 ...
 <Trash2 size={16} />
  ...
  {!readOnly && (
 <Button variant="secondary"onClick={addLine} size="sm"className="mt-4 w-full border-dashed border-2 hover:border-blue-400 hover:bg-blue-50 dark:bg-blue-500/10">
