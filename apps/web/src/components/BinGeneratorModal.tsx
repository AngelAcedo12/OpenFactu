...
export const BinGeneratorModal: React.FC<BinGeneratorModalProps> = ({ warehouseId, onSuccess, onClose }) => {
 ...
 const total = (aisleRange.end - aisleRange.start + 1) *  (stackRange.end - stackRange.start + 1) *  (levelRange.end - levelRange.start + 1);

 return (
 <Modal  isOpen={true}  onClose={onClose}  title="Generador Masivo de Ubicaciones (Motor Industrial)" subtitle="Crea mallas de almacenamiento complejas al instante mediante parámetros jerárquicos." maxWidth="2xl" >
 ...
 <Input  label="Prefijo" placeholder="Ej: B, P, R..." value={prefix}  onChange={(e) => setPrefix(e.target.value)}  />
 <Input  label="Separador" placeholder="Ej: -, /, ." value={separator}  onChange={(e) => setSeparator(e.target.value)}  />
 </div>
 ...
 <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500">Pasillo (Aisle)</p>
 ...
 <div className="space-y-2">
 <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500">Columna (Stack)</p>
 <div className="flex gap-2">
 ...
 <div className="space-y-2">
 <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500">Nivel (Level)</p>
 <div className="flex gap-2">
 ...
 <Button  onClick={handleGenerate}  disabled={isGenerating || total <= 0}  className="shadow-lg" >
