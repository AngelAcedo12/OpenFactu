...
import {
 FileText, Users, Building2, ListOrdered, Clock, Search, Copy, Code2,
 Type, Hash, ToggleLeft, Calendar, Box, List
} from 'lucide-react';
...
const TYPE_COLORS: Record<string, string> = {
 ...
 number: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 border-emerald-100 dark:border-emerald-500/20',
...
export const FieldExplorer: React.FC<Props> = ({ onInsert, insertMode }) => {
 ...
 placeholder="Buscar campo..." className="pl-7 h-8 text-xs" />
 </div>
 ...
 <span className="text-slate-500 dark:text-slate-400 dark:text-slate-500">{GROUP_ICON[group.icon]}</span>
 <span className="text-[10px] font-black text-slate-600 dark:text-slate-300 dark:text-slate-600 uppercase tracking-wider">{group.label}</span>
 </div>
 {group.note && (
 ...
 <button
 key={field.path}
 type="button" onClick={() => handleFieldClick(field)}
...
"text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border inline-flex items-center gap-0.5 flex-shrink-0",
 TYPE_COLORS[field.type] || 'bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 dark:text-slate-600 border-slate-200 dark:border-slate-700'
 )}>
 {TYPE_ICON[field.type]} {field.type}
 </span>
 </div>
 <div className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-0.5 leading-tight">{field.description}</div>
 {field.example && (
 <div className="text-[9px] text-slate-400 dark:text-slate-500 italic mt-0.5 font-mono">ej: {field.example}</div>
 )}
 </div>
 <Copy size={11} className="text-slate-300 dark:text-slate-600 group-hover:text-indigo-500 flex-shrink-0 mt-0.5"/>
 </div>
 </button>
 ))}
 </div>
 ...
 <span className="text-[10px] font-black text-indigo-700 uppercase tracking-wider">Helpers Handlebars</span>
 </div>
 {HELPERS.map((helper) => (
 <div key={helper.name} className="px-3 py-2 border-b border-slate-50 last:border-b-0">
 <code className="text-[11px] font-mono font-semibold text-indigo-700 block break-all">{helper.usage}</code>
 <div className="text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-0.5">{helper.description}</div>
 </div>
 ))}
 </div>
 ...
 No hay campos que coincidan con"{search}" </div>
 )}
 </div>
 </div>
 );
