...
export const TemplatesList: React.FC<Props> = ({
 data, loading, onCreate, onEdit, onSetDefault, onDuplicate, onDelete
}) => {
 ...
 {item.isDefault && <Badge variant="success"className="text-[9px] font-black uppercase">Default</Badge>}
 </div>
 )
 },
 {
 header: 'Última actualización',
 accessor: (item: TemplateRow) => item.updatedAt ? new Date(item.updatedAt).toLocaleString('es-ES') : '—'
 },
 {
 header: 'Acciones',
 ...
 {!item.isDefault && (
 <Button variant="ghost"size="sm"onClick={(e: any) => { e.stopPropagation(); onSetDefault(item); }} title="Marcar como default">
 <Star size={14} />
 </Button>
 )}
 <Button variant="ghost"size="sm"onClick={(e: any) => { e.stopPropagation(); onDuplicate(item); }} title="Duplicar">
 <Copy size={14} />
 </Button>
 <Button variant="ghost"size="sm"onClick={(e: any) => { e.stopPropagation(); onDelete(item); }} title="Borrar"className="text-rose-500 hover:text-rose-700">
 <Trash2 size={14} />
 </Button>
 </div>
 )
 ...
 <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2 font-medium ml-1">Formatos PDF personalizables para facturas, albaranes y pedidos.</p>
 </div>
 <Button onClick={onCreate} className="flex items-center gap-2 shadow-xl shadow-indigo-500/10 h-12 px-6">
 ...
 {!loading && (Object.keys(DOC_TYPE_LABELS) as DocType[]).map((docType) => {
 const rows = grouped[docType] || [];
 return (
 <Card key={docType} className="overflow-hidden shadow-lg border-slate-100 dark:border-slate-800"noPadding>
 <div className={`px-6 py-3 border-b flex items-center justify-between ${DOC_TYPE_COLORS[docType]}`}>
 <div className="flex items-center gap-2">
 <span className="text-[10px] font-black uppercase tracking-widest">{docType}</span>
