...
export const PluginFields: React.FC<PluginFieldsProps> = ({ tableName, values, onChange, disabled }) => {
 ...
 <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-2">Campos Adicionales (Plugins)</h4>
 </div>
 {fields.map((f) => (
 <div key={f.id} className="space-y-1">
 <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-tight">{f.label}</label>
 {f.fieldType === 'BOOLEAN' ? (
 <div className="flex items-center h-10 gap-2">
 <input  type="checkbox" checked={!!values[f.fieldName]}  onChange={(e) => onChange(f.fieldName, e.target.checked)}
 disabled={disabled}
 className="w-4 h-4 accent-blue-600" />
 <span className="text-xs font-medium text-slate-600 dark:text-slate-300 dark:text-slate-600">Activar</span>
 </div>
 ) : (
 <Input
